...
- Train a small neural network or regression model on simulation data:
  ...
  \dot{x}_{true} = f_{model}(x,u) + f_{learned}(x,u)

  \]

- Integrate \( f_{learned} \) inside MPC’s prediction model.
...
**Keywords:** MPC, Lyapunov stability, convex optimization, spacecraft control, learning-based control, successive convexification, trajectory optimization
