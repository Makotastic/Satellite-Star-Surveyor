...
error_mapping = ErrorStateMappingService()
...
def simulate_closed_loop(
    ...
    for step in tqdm(range(num_steps), desc=f"Simulating {controller_name}"):
        ...
        if controller_name == "lqr":
            error_state = error_mapping.state_error(state, goal_state)
...
# Save all results
all_results = {
    name: {**results[name], "metrics": comparison_metrics[name]} for name in results
}
