...
# TODO Make full body state slices, integtrate into utils.py
IDX_POS = slice(0, 3)
IDX_VEL = slice(3, 6)
IDX_THETA = slice(6, 10)
...
class StarPlanner:
    ...
    def tick(self, body_state: FloatArray, dt: float, sun_dir_I: Vec3) -> tuple[Vec3 | None, PlanModes, bool]:
        ...
        if self.mode == PlanModes.NO_TARGET:
            star_vec, _ = self._update_target(body_state, earth_dir_I, sun_dir_I, body_dir_I)
            if star_vec is None:
                self.mode = PlanModes.NO_TARGET
            else:
        ...
        feasible, _, _ = self._is_target_feasible(
                                        earth_angle, 
                                        sun_angle, 
                                        body_state
                                        )
        ...
        if dist_angle > self.stabilize_threashold:
            self.mode = PlanModes.SLEWING

        if dist_angle <= self.stabilize_threashold:
            self.mode = PlanModes.SETTLING

        if dist_angle <= self.observe_threashold and mag_omega < self.observe_rate_threashold:
...
def convert_ECI_pos_to_unit_vec(targets: pd.DataFrame) -> pd.DataFrame:
    ...
    unit_targets = targets.assign( x = np.cos(targets.Dec) * np.cos(targets.RA), 
