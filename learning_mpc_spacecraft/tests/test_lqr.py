...
def A_B(dynamics, ref_state):
    ...
    zero_u = np.zeros(3)
    return dynamics.linearize(ref_state, zero_u)
...
@pytest.mark.unit
def test_saturation(lqr_discrete, small_error_state):
    ...
    u_sat = lqr_discrete.compute_control_with_saturation(small_error_state, u_min=u_min, u_max=u_max)
    # If u_unsat exceeds bounds, should be clipped
    if np.any(u_unsat > u_max):
        assert np.all(u_sat <= u_max)
    if np.any(u_unsat < u_min):
        assert np.all(u_sat >= u_min)

@pytest.mark.unit
def test_compute_control_with_ref_matches_error_definition(lqr_discrete, small_error_state):
    """
...
def test_closed_loop_convergence(dynamics, ref_state, Q, R, small_error_state):
    """Integration test: closed-loop simulation with LQR, verify convergence."""
    A, B = dynamics.linearize(ref_state, np.zeros(3))
    Ad, Bd = dynamics.discretize_linear_system(A, B)
    ...
    for _ in range(100):  # 10 seconds
        ...
        u = lqr.compute_control(error_state)
    ...
    q_final = np.quaternion(*states[-1][:4])
    assert np.isclose(q_final.norm(), 1.0, atol=1e-10)


@pytest.mark.integration
def test_lqr_with_disturbance(dynamics, ref_state, Q, R, small_error_state):
