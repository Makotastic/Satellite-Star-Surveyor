...
from mpc_spacecraft.utilities.utils import FloatArray, Quat
...
class ScenarioGenerator:
    ...
    @staticmethod
    def slew_maneuver(
        q_initial: FloatArray, q_final: FloatArray, duration: float, dt: float
    ) -> Callable:
        """
        ...
        q_initial = qu.quaternion(*q_initial).normalized()

        q_final = qu.quaternion(*q_final).normalized()

        def reference_trajectory(t: float) -> FloatArray:
            """Return reference state at time t using SLERP."""

            q_ref = qu.quaternion.slerp(q_initial, q_final, 0, duration, t).normalized()

            q_ref = qu.quaternion.as_float_array(q_ref)
    ...
    def constant_rate_spin(
        omega_target: FloatArray, duration: float, dt: float
    ) -> Callable:
        """
        ...
        def reference_trajectory(t: float) -> FloatArray:
            ...
            q_ref = qu.quaternion.integrate_angular_velocity(omega_target, 0, t)
    ...
    def tracking_trajectory(
        waypoints: list[FloatArray], times: list[FloatArray], dt: float
    ) -> Callable:
        """
        ...
        def reference_trajectory(t: float) -> np.ndarray:
            ...
            if t <= times[0]:
                ...
                x_q_0 = qu.quaternion(*x0[:4]).normalized()
                x_q_1 = qu.quaternion(*x1[:4]).normalized()
                x_q_ref = qu.quaternion.slerp(x_q_0, x_q_1, t0, t1, t).normalized()
                x_q_ref = qu.quaternion.as_float_array(x_q_ref).squeeze()
...
def create_reference_trajectory(scenario_type: str, **kwargs) -> Callable:
    """
